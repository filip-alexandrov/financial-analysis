...
let fastMa = [];
let slowMa = [];
tulind.indicators.ema.indicator([close], [fastMaLength], function (err, res) {
  fastMa.push(...res[0]);
});
tulind.indicators.ema.indicator([close], [slowMaLength], function (err, res) {
  slowMa.push(...res[0]);
});

let val = math.multiply(
  100,
  math.dotDivide(math.subtract(fastMa, slowMa), slowMa)
);

let min = [];
let max = [];
tulind.indicators.min.indicator([val], [lookBackPeriod], function (err, res) {
  for (let i = 0; i < tulind.indicators.min.start([lookBackPeriod]); i++) {
    min.push(0);
  }
  min.push(...res[0]);
});
tulind.indicators.max.indicator([val], [lookBackPeriod], function (err, res) {
  for (let i = 0; i < tulind.indicators.max.start([lookBackPeriod]); i++) {
    max.push(0);
  }
  max.push(...res[0]);
});

let range = math.subtract(max, min);
let flUp = math.dotDivide(math.multiply(upLevel, range), 100);
let flDown = math.dotDivide(math.multiply(downLevel, range), 100);
let histo = math.abs(math.subtract(flUp, flDown));
...
console.log(math.matrixFromColumns(close, histo, bufferLine));
